...
import { articleIndexability, isArticlePublic, publicFAQItems } from './content/model';
...
export const INDEXABLE_PATHS = Object.entries(CUSTOM).filter(([, meta]) => meta.indexability === 'index').map(([path]) => path)
  .concat(ARTICLES.filter(isArticlePublic).map((article) => article.slug));
...
export function getPageSeo(inputPath: string): SeoData {
  ...
  if (article) return { title: article.metaTitle, description: article.metaDescription, canonical: article.canonical, indexability: articleIndexability(article), type: 'article' };
...
export function schemaForPath(inputPath: string): object[] {
  ...
  if (path === '/faq/') {
    const approvedFAQ = ARTICLES.flatMap(publicFAQItems);
    if (approvedFAQ.length) schemas.push({ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: approvedFAQ.map((item) => ({ '@type': 'Question', name: item.question, acceptedAnswer: { '@type': 'Answer', text: item.answer } })) });
  }
  const article = articleBySlug(path);
  if (article && isArticlePublic(article)) {
    ...
    const approvedFAQ = publicFAQItems(article);
    if (article.schema.includes('FAQPage') && approvedFAQ.length) schemas.push({ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: approvedFAQ.map((item) => ({ '@type': 'Question', name: item.question, acceptedAnswer: { '@type': 'Answer', text: item.answer } })) });
  }
